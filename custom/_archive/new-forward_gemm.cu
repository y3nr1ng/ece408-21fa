...
__host__ void GPUInterface::conv_forward_gpu(float* device_y,
  ...
  dim3 grid_unroll(ceil((float)W_out / block.x),
                   ceil((float)H_out / block.y),
                   ceil((float)B / block.z));
  dim3 grid_mma(ceil((float)H_out * W_out / block.x),
  ...
  printf("*** grid=(x=%d, y=%d, z=%d), block=(x=%d, y=%d, z=%d)\n",
         grid_unroll.x, grid_unroll.y, grid_unroll.z, block.x, block.y, block.z);
  im2col<<<grid_unroll, block, smem_size_unroll>>>(device_xc, device_x,
                                                   B, M, C, H, W, K);

  // Call the kernel
  printf("*** grid=(x=%d, y=%d, z=%d), block=(x=%d, y=%d, z=%d)\n",
         grid_mma.x, grid_mma.y, grid_mma.z, block.x, block.y, block.z);
  matrix_multiply<<<grid_mma, block, smem_size_gemm>>>(device_y, device_xc,
                                                       B, M, C, H, W, K);
}
