...
#define t2d(i, i_hw, i_ckk)                    \
  tile[(t_b) * (2 * TILE_WIDTH * TILE_WIDTH) + \
       ...
       (i_ckk)]
#define k1d(i_ckk)                                 \
  kernel[(m) * (C * KERNEL_WIDTH * KERNEL_WIDTH) + \
         (i_ckk)]
...
#define t2d_kt(i_m, i_ckk) \
  ...
  const int n_kernel = C * KERNEL_WIDTH * KERNEL_WIDTH;
  ...
  if ((b < B)) {
    ...
    for (int n = 0; n < n_tiles; n++) {
      ...
      if ((dst_hw < n_hw) && (dst_ckk < n_kernel)) {
        ...
        const int q = dst_ckk % KERNEL_WIDTH;
        dst_ckk /= KERNEL_WIDTH;
        const int p = dst_ckk % KERNEL_WIDTH;
        const int c = dst_ckk / KERNEL_WIDTH;

        t2d_xc(t_m, t_hw) = __float2half(x3d(c, dst_y + p, dst_x + q));
      } else {
        t2d_xc(t_m, t_hw) = (half)0.0;
      }
      dst_ckk = n * blockDim.x + t_hw;
      if ((m < M) && (dst_ckk < n_kernel)) {
        t2d_kt(t_m, t_hw) = __float2half(k1d(dst_ckk));
      } else {
        t2d_kt(t_m, t_hw) = (half)0.0;
      }
      __syncthreads();

      wmma::load_matrix_sync(a_frag, &t2d_kt(0, 0), TILE_WIDTH);
      wmma::load_matrix_sync(b_frag, &t2d_xc(0, 0), TILE_WIDTH);
...
#pragma unroll
      for (int k = 0; k < TILE_WIDTH; k++) {
        acc = __hfma(t2d_kt(t_m, k), t2d_xc(k, t_hw), acc);
      }
      __syncthreads();
      */
    }
    ...
    if ((m < M) && (dst_hw < n_hw)) {
      //y1d(dst_hw) = __half2float(acc);
      wmma::store_matrix_sync(&y1d(dst_hw), acc_frag, TILE_WIDTH, wmma::mem_col_major);
    }
  }
...
/*** Prolog BEGIN ***/
  ...
  size_t smem_size =
      B_BATCH * 2 * PADDED_TILE_WIDTH * PADDED_TILE_WIDTH * sizeof(half);
